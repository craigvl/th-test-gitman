...
//     val next = this.remove(m).descendingByRetweet;
//     if(next.isEmpty) return new Cons(m,next)
//     else return new Cons(m,this.remove(m).descendingByRetweet)
//    
     //descendingByRetweetAcc(mostRetweeted, this.remove(mostRetweeted), new Cons(mostRetweeted, Nil))
  
  
//  def descendingByRetweetAcc(tw: Tweet, acc: TweetList): TweetList = {    
//    else {
//      val most = if(tw.retweets > c.head.retweets) tw.retweets else 
//      descendingByRetweetAcc(most, c.remove(mr), tl :: TweetList(mr))
//    }
//  }

      
    //  val m=this.mostRetweeted
    //  val l = new Cons(m,this.remove(m).descendingByRetweet)
//      if(l.isEmpty) return new Cons(m, new Empty);
//      else return new Cons(m,this.remove(m).descendingByRetweet)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
 // def isEmpty = false
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =  {
   ...
   def descendingByRetweet: TweetList = {
//     val next = this.remove(elem).descendingByRetweet;
//     if(next.isEmpty) return Nil
      val max = this.mostRetweeted;
      return new Cons(max,this.remove(max).descendingByRetweet)
  ...
  def mostRetweeted : Tweet = { 
//     def getMax(tweets: NonEmpty, max : Tweet) : Tweet = {
//        if(elem.retweets < max.retweets) getMax(right, getMax(left, getMax(max));
//        else getMax(left.getMax(elem)); 
//     }
...
object GoogleVsApple {
  ...
  def getTweetsByKey(keys : List[String]) =  
    TweetReader.allTweets
      .filter(v => keys.map(c => c.toLowerCase())
          .contains(v.text.toLowerCase()));
  
  // google.union(apple) foreach println

    lazy val googleTweets: TweetSet = getTweetsByKey(google);
    lazy val appleTweets: TweetSet = getTweetsByKey(apple);
