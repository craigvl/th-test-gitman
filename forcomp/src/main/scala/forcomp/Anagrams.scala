...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences =  w.groupBy(g => g.toLower).map({ case (k,v) => (k,v.length()) }).toList.sortBy(c => c._1);

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = wordOccurrences(s.mkString)
  //List(("xxaa"),("ffaa")).mkString
  ...
  def wordAnagrams(word: Word): List[Word] =  dictionaryByOccurrences.get(
      wordOccurrences(word)) match {
              case Some(ws) => ws
              case None => List[Word]()
        }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
      def sentenceAnagramsIter(occurrences: Occurrences): List[Sentence] = {
        if (occurrences.isEmpty) {
          ...
          val cs = combinations(occurrences)
          val ps = cs.flatMap(c => permutations(c))
          //val cs = combinations(occurrences)
          for (i <- ps if dictionaryByOccurrences.keySet(i);
               j <- dictionaryByOccurrences(i);
               s <- sentenceAnagramsIter(subtract(occurrences, i))) yield {j :: s}
        }
      }
